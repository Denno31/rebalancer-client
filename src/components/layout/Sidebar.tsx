...
const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  return (
    <div className={`fixed top-0 left-0 h-full bg-[#1E2026] text-white z-10 transition-all duration-300 ${collapsed ? 'w-16' : 'w-56'} shadow-lg`}>
      ...
      <nav className="mt-4">
        <Link href="/" passHref>
          <div className={`flex items-center gap-3 px-4 py-3 cursor-pointer ${isActive('/') ? 'bg-gray-700 text-white' : 'text-gray-400 hover:bg-gray-700 hover:text-white'}`}>
        ...
        <Link href="/system-config" passHref>
          <div className={`flex items-center gap-3 px-4 py-3 cursor-pointer mt-1 ${isActive('/system-config') ? 'bg-gray-700 text-white' : 'text-gray-400 hover:bg-gray-700 hover:text-white'}`}>
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
